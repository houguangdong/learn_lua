...
local function launch_slave(auth_handler)
    local function auth(fd, addr)
        ...
        local clientkey = crypt.base64encode(handshake)
        if #clientkey ~= 8 then
        ...
        local respone = assert_socket("auth", socket.readline(fd), fd)
...
local function login(conf)
    ...
    skynet.start(function()
        ...
        if loginmaster then
            ...
            launch_master = nil
            conf = nil
            launch_slave(auth_handler) -- 启动从属
        else
            launch_slave = nil
