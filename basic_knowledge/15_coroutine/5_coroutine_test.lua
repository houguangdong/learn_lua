...
--coroutine.creat返回的是一个协同程序，类型为thread,需要使用coroutine.resume进行调用；
--而coroutine.wrap返回的是一个普通的方法(函数)，类型为function，和普通function有同样的使用方法，并且不能使用coroutine.resume进行调用。
--以下代码进行测试：
...
print('------------------------------------create和resume--------------------------------------------------------------')
--coroutine.create方法只要建立了一个协程 ，那么这个协程的状态默认就是suspend。
--使用resume方法启动后，会变成running状态；遇到yield时将状态设为suspend；如果遇到return，那么将协程的状态改为dead。
--
--coroutine.resume方法需要特别注意的一点是，这个方法只要调用就会返回一个boolean值。
